...
Original file is located at
    https://colab.research.google.com/drive/1_qtOCQQj8sc8V0FUMxVG8_-a0CzrAT3Z
"""
...
def to_tensor(image):
    ...
    # If the image has 4 channels (e.g., RGBA), convert it to RGB
    if img.shape[-1] == 4:
        img = img[..., :3]  # Keep only the first 3 channels (RGB)

    # Ensure the tensor has 3 dimensions (C, H, W)
    if len(img.shape) == 2:  # If grayscale
        img = np.stack([img, img, img], axis=-1)  # Convert grayscale to 3 channels
