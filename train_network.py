...
def main(cfg: DictConfig):
    ...
    for num_epoch in range((cfg.opt.iterations + 1 - first_iter) // len(dataloader) + 1):
        ...
        for data in dataloader:
            ...
            back_output = gaussian_predictor(back_layer)

            front_loss = loss_fn(front_output, data['target_front'])
            back_loss = loss_fn(back_output, data['target_back'])
            ...
            total_loss = combined_loss * lambda_l12  # Apply weighting for L1/L2 loss
            if cfg.opt.lambda_lpips != 0:
                # Compute LPIPS loss separately if enabled
                lpips_loss_front = lpips_fn(front_output * 2 - 1, data['target_front'] * 2 - 1)
                lpips_loss_back = lpips_fn(back_output * 2 - 1, data['target_back'] * 2 - 1)
